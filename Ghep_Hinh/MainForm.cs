    ...
    public partial class MainForm : XtraForm
    {
        ...
        private void MainForm_ClientSizeChanged(object sender, EventArgs e)
        {
            if (_previousClientWidth != this.ClientSize.Width || _previousClientHeight != this.ClientSize.Height)
            {
                ...
                DisplayImage(Settings.Default.ShowImageHint);                
            }
        }
            ...
            if (_canMovePiece)
            {
                ...
                if (_clusters.Count == 1)
                {
                    if (_victoryAnnounced == false)
                    {
						...
						DevExpress.XtraEditors.XtraMessageBox.Show("Hình đã được ghép hoàn chỉnh. Xin chúc mừng bạn!", "Laptrinhvb.net", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
        ...
        private void OpenFileMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                {                    
                    ...
                    ResponseMessage responseMsg = CreateMsg();

                    if (!responseMsg.Okay)
                    {
                        _sourcePicture.Dispose();
						DevExpress.XtraEditors.XtraMessageBox.Show(responseMsg.Message, responseMsg.Caption, MessageBoxButtons.OK, MessageBoxIcon.Warning );
                        return;
                    }
        ...
        private ResponseMessage CreateMsg()
        {
            if (_sourcePicture == null)
            {
                return new ResponseMessage
                {
                    Message = "Xin vui lòng chọn hình."
                };
            }
            ...
            if (pieceWidth < GameSettings.MIN_PIECE_WIDTH || pieceHeight < GameSettings.MIN_PIECE_HEIGHT)
            {
                return new ResponseMessage
                {
                    Message = "Hình quá nhỏ. Vui lòng chọn hình lớn hơn."
                };                
            }
        ...
        private ResponseMessage DisplayImage(bool showGhostPicture)
        {
            if (_sourcePicture == null)
            {
                return new ResponseMessage
